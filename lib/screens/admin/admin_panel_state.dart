...
mixin AdminPanelStateMixin<T extends StatefulWidget> on State<T> {
  ...
  Map<String, List<timetable_model.Timetable>> _teacherTimetables = {};
  ...
  Future<void> loadDashboardData(BuildContext context) async {
    ...
    if (currentSchool != null) {
      try {
        ...
        classNameMap[null] = l10n.unassigned;
  ...
  Future<void> _updateAllTeachersScheduleSummaries() async {
    ...
    if (currentSchool != null) {
      try {
        ...
        if(mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("${l10n.errorOccurredPrefix}: ${e.toString()}")));
      }
    }
  }
}
